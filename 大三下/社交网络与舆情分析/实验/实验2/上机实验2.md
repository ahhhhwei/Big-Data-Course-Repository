...
- 对提取出来的所有权限做交集，取前20个权限；

- 根据提取的权限，生成一个关于`apk`的关系图

**实验要求：**

- 生成`apk`的关系图；
- 根据权限关系图比较两个`apk`的相似度。

## 实验分析

`apk`之间的关系是指，以`apk`文件为节点，以`apk`权限为边的关系图，而相似度，我用了类似于`Jaccard`相似度的度量方法：
$$
相似度=\dfrac{两个apk共享权限数目}{两个apk总权限数目}
$$
